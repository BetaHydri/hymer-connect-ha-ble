    ...
    async def start_ble(self) -> bool:
        ...
        try:
            ...
            _LOGGER.warning(
                ...
                "TLS will fail on this link. Recover with 'systemctl restart "
                "bluetooth' on the Home Assistant host (or a host reboot). BLE "
                "will retry slowly; the cloud path keeps working. Detail: %s",
                err,
            )
            if self._ble_client:
                try:
