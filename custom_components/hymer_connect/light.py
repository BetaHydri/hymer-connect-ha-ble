...
class HymerConnectLight(
    ...
    def available(self) -> bool:
        ...
        # slot discovery can surface a phantom raw bus-3 value (int 1) that must
        # NOT be misread as "12V off" (#20, @stbcgn). Genuine 12V-off is still
        # caught by the data-silence check below.
        if isinstance(main, str) and main == "Off":
            return False
        # Double bottom: an explicit SCU standby (scu_connected=False) means 12V
        # is off even when the main_switch readback was momentarily reset to None
        # during a SignalR reconnect and the standby frames keep the data-silence
        # clock alive. Only an explicit False greys out - None (never observed /
        # mid-reconnect) and True (12V still on, incl. the ~60s command-verify
        # window) must NOT.
        if _resolve_path(
