                        ...
                        for p in msg.data.split('\x1e'):
                            ...
                            if pp.get('type')==6:
                                ...
                                if a and isinstance(a[0],str):
                                    new = pia.decode_pia_payload(a[0])
                                    sensors.update(new)
                                    resp_count += 1
                                    print(f'  R#{resp_count}: +{len(new)} = {len(sensors)}', flush=True)
                            elif pp.get('type')==1:
                                inv = pp.get('invocationId','?')
                                err = pp.get('error','')
                                print(f'  INV {inv}: err={err}', flush=True)
                    else:
                        print(f'  WS MSG TYPE: {msg.type}', flush=True)
                        break
            except asyncio.TimeoutError: print('  TIMEOUT', flush=True)
            print(f'=== {len(sensors)} sensors ===', flush=True)
