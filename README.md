...
- [@and0nna](https://github.com/and0nna) — HYMER **B-MC I 680** BLE field testing and the clear multi-attempt diagnosis behind the **re-pair stale-bond fix** (a bond left from an attempt aborted before the token was minted blocked every later re-pair) and the stale-write-channel recovery guidance ([#16](https://github.com/BetaHydri/hymer-connect-ha-ble/issues/16), [#29](https://github.com/BetaHydri/hymer-connect-ha-ble/issues/29)).
