...
| [`scan_pia_fields.py`](scan_pia_fields.py) | **Single-shot** PIA protobuf field probe. Sends **one** operator-chosen `UserRequestTopic` / `CommandRequestTopic` sub-field per run to identify an undocumented command (`getPairedMobileDevices`, `deleteMobileDevices`, `deleteUser`, `deleteAllUsers`). **Never sweeps a range** — the `delete*` field numbers are unknown and a blind sweep could wipe the account, so it refuses to send without `--i-understand-may-be-destructive` and blocks the pairing (4/6) and restart (command 2) fields. Prefer probing the read-only `getPairedMobileDevices` first (empty payload). Same safety model as `dan-simms1/hymer-connect-ha`'s `scu-user-topic`. **Reverse-engineering tool, not for daily use.** | `python tools/scan_pia_fields.py --envelope user --field 5 --i-understand-may-be-destructive` |
